...
class CustomDimensionPlan(ExecutionPlan):
    ...
    def execute(self, client, params, start_time):
        ...
        # Step 1 — Create the custom dimension type
        result = client.post(
        ...
        # Step 2 — Create each dimension value; capture the ID for the one used in the voucher
        ...
        for value_name in dimension_values:
            ...
            if value_name == voucher_dimension_value:
                voucher_value_id = result["body"]["value"]["id"]

        if voucher_value_id is None:
            # Fallback: if exact match not found (e.g. case mismatch), use the first created value
            raise RuntimeError(
                f"Dimension value '{voucher_dimension_value}' not found among created values: "
                f"{dimension_values}"
            )
